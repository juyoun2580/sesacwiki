...
function renderMockInterview(c) {
  ...
  const tabsHTML = `
    <div class="ff-tabs ff-tabs--sm ff-mock__view-tabs">
      <button type="button" class="ff-tab${_mockView === 'practice' ? ' ff-tab--active' : ''}" data-mv="practice">🎤 연습하기</button>
      <button type="button" class="ff-tab${_mockView === 'overview' ? ' ff-tab--active' : ''}" data-mv="overview">📋 전체 답변 보기</button>
    </div>`;

  // ── 전체 답변 보기 ──────────────────────────────────────────────────────
  if (_mockView === 'overview') {
    const catColors = { '기본': 'gray', '경험': 'blue', '비전': 'green', '직무': 'orange' };
    const mockAnswers = f.mockAnswers || {};
    const totalAnswered = allQ.filter(q => (mockAnswers[q.id] || []).length > 0).length;

    c.innerHTML = `
      <div class="ff-mock">
        ${tabsHTML}
        <div class="ff-mock-overview">
          <p class="ff-mock-overview__summary">총 ${allQ.length}개 질문 중 <strong>${totalAnswered}개</strong>에 답변이 저장되어 있어요.</p>
          ${allQ.map((q, i) => {
            const answers = (mockAnswers[q.id] || []).slice().reverse();
            return `
              <div class="ff-mock-overview__item">
                <div class="ff-mock-overview__q-row">
                  <span class="tag tag--${catColors[q.category] || 'gray'}">${esc(q.category)}</span>
                  <p class="ff-mock-overview__q">Q${i + 1}. ${esc(q.q)}</p>
                  <button type="button" class="btn btn--outline btn--sm ff-mock-ov-go" data-qi="${i}">연습하기</button>
                </div>
                ${answers.length ? `
                  <div class="ff-mock-overview__answers">
                    ${answers.map(a => `
                      <div class="ff-mock-overview__answer">
                        <div class="ff-mock-overview__answer-meta">
                          <span class="ff-mock-overview__answer-date">${fmtSavedAt(a.savedAt)}</span>
                          <button type="button" class="btn btn--ghost btn--sm ff-mock-ov-del" data-qid="${q.id}" data-aid="${a.id}">삭제</button>
                        </div>
                        <p class="ff-mock-overview__answer-text">${esc(a.text)}</p>
                      </div>`).join('')}
                  </div>` :
                  `<p class="ff-mock-overview__no-answer">아직 저장된 답변이 없어요.</p>`}
              </div>`;
          }).join('')}
        </div>
      </div>`;

    c.querySelectorAll('[data-mv]').forEach(btn => btn.onclick = () => {
      _mockView = btn.dataset.mv;
      renderMockInterview(c);
    });
    c.querySelectorAll('.ff-mock-ov-go').forEach(btn => btn.onclick = () => {
      _mockIdx  = parseInt(btn.dataset.qi, 10);
      _mockView = 'practice';
      renderMockInterview(c);
    });
    c.querySelectorAll('.ff-mock-ov-del').forEach(btn => btn.onclick = () => {
      const { qid, aid } = btn.dataset;
      const f2 = loadFeatures();
      if (!f2.mockAnswers) f2.mockAnswers = {};
      f2.mockAnswers[qid] = (f2.mockAnswers[qid] || []).filter(a => a.id !== aid);
      saveFeatures(f2);
      T('답변을 삭제했어요.');
      renderMockInterview(c);
    });
  ...
  const savedAnswers = ((f.mockAnswers || {})[q.id] || []).slice().reverse();
  const catColors = { '기본': 'gray', '경험': 'blue', '비전': 'green', '직무': 'orange' };
  ...
  c.innerHTML = `
    <div class="ff-mock">
      ...
      <div class="ff-mock__write-zone">
        <label class="ff-mock__write-label" for="mock-write-ta">✏️ 새 답변 작성</label>
        <textarea id="mock-write-ta" class="ff-textarea ff-mock__write-ta" rows="5"
                  placeholder="음성 인식 결과가 여기에 채워지거나, 직접 답변을 입력할 수 있어요."></textarea>
        <div class="ff-mock__write-footer">
          <span class="ff-char-count" id="mock-write-count">0자</span>
          <button type="button" class="btn btn--primary btn--sm" id="mock-save-btn">💾 저장</button>
        </div>
      </div>
      ${savedAnswers.length ? `
        <div class="ff-mock__saved-list">
          <p class="ff-mock__saved-title">저장된 답변 <span class="ff-mock__saved-count">${savedAnswers.length}개</span></p>
          ${savedAnswers.map(a => `
            <div class="ff-mock__saved-item">
              <div class="ff-mock__saved-item-meta">
                <span class="ff-mock__saved-item-date">${fmtSavedAt(a.savedAt)}</span>
                <button type="button" class="btn btn--ghost btn--sm ff-mock__saved-del" data-aid="${a.id}">삭제</button>
              </div>
              <p class="ff-mock__saved-item-text">${esc(a.text)}</p>
            </div>`).join('')}
        </div>` : ''}
  ...
  if (SR) {
    ...
    function startRec() {
      ...
      recognition.onresult = (e) => {
        ...
        const ta = c.querySelector('#mock-write-ta');
        if (ta) { ta.value = finalText + (interim ? ' ' + interim : ''); c.querySelector('#mock-write-count').textContent = ta.value.length + '자'; }
      };
  ...
  writeTa.oninput = () => { writeCount.textContent = writeTa.value.length + '자'; };

  c.querySelector('#mock-save-btn').onclick = () => {
    const text = writeTa.value.trim();
    if (!text) return T('저장할 답변을 입력해주세요.');
    const f2 = loadFeatures();
    if (!f2.mockAnswers) f2.mockAnswers = {};
    if (!f2.mockAnswers[q.id]) f2.mockAnswers[q.id] = [];
    f2.mockAnswers[q.id].push({ id: genId(), text, savedAt: new Date().toISOString() });
    saveFeatures(f2);
    writeTa.value = '';
    writeCount.textContent = '0자';
    T('답변을 저장했어요! ✅');
    renderMockInterview(c);
  };

  c.querySelectorAll('.ff-mock__saved-del').forEach(btn => btn.onclick = () => {
    const f2 = loadFeatures();
    if (!f2.mockAnswers) f2.mockAnswers = {};
    f2.mockAnswers[q.id] = (f2.mockAnswers[q.id] || []).filter(a => a.id !== btn.dataset.aid);
    saveFeatures(f2);
    T('답변을 삭제했어요.');
    renderMockInterview(c);
  });
...
// ── 21. 면접 준비 통합 패널 ───────────────────────────────────────────────
let _interviewMgrTab = 'questions';

function renderInterviewManager(c) {
  const tabs = [
    { key: 'questions', label: '❓ 예상 질문' },
    { key: 'answers',   label: '📝 답변 스크립트' },
    { key: 'mock',      label: '🎬 모의 면접' },
  ];

  c.innerHTML = `
    <div class="ff-tabs ff-tabs--sm" style="margin-bottom:var(--space-20)">
      ${tabs.map(t => `
        <button type="button" class="ff-tab${_interviewMgrTab === t.key ? ' ff-tab--active' : ''}" data-imtab="${t.key}">${t.label}</button>
      `).join('')}
    </div>
    <div id="ff-intmgr-body"></div>`;

  c.querySelectorAll('[data-imtab]').forEach(btn => btn.onclick = () => {
    _interviewMgrTab = btn.dataset.imtab;
    renderInterviewManager(c);
  });

  const body = c.querySelector('#ff-intmgr-body');
  if (_interviewMgrTab === 'questions') renderInterviewQuestions(body);
  else if (_interviewMgrTab === 'answers') renderInterviewAnswers(body);
  else renderMockInterview(body);
}

// ── 22. 지원 관리 통합 패널 ───────────────────────────────────────────────
let _appMgrTab = 'companies';

function renderApplicationManager(c) {
  const tabs = [
    { key: 'companies', label: '🏢 기업 목록' },
    { key: 'schedule',  label: '📅 면접 일정' },
  ...
  c.innerHTML = `
    <div class="ff-tabs ff-tabs--sm" style="margin-bottom:var(--space-20)">
      ${tabs.map(t => `
        <button type="button" class="ff-tab${_appMgrTab === t.key ? ' ff-tab--active' : ''}" data-amtab="${t.key}">${t.label}</button>
      `).join('')}
    </div>
    <div id="ff-appmgr-body"></div>`;

  c.querySelectorAll('[data-amtab]').forEach(btn => btn.onclick = () => {
    _appMgrTab = btn.dataset.amtab;
    renderApplicationManager(c);
  });

  const body = c.querySelector('#ff-appmgr-body');
  if (_appMgrTab === 'companies') renderCompanies(body);
  else if (_appMgrTab === 'schedule') renderInterviewSchedule(body);
...
const JOB_FEATURES = {
  ...
  'interview-manager':   { title: '면접 준비',          render: renderInterviewManager },
  'research':            { title: '기업 / 직무 리서치', render: renderResearch },
  'application-manager': { title: '지원 관리',          render: renderApplicationManager },
};
